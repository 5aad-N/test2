...
def api_update_profile(request: HttpRequest) -> JsonResponse:
    ...
    Accepts:
        ...
        - date_of_birth: str (optional, YYYY-MM-DD format)
        - profile_image: File (optional)
        - currency_preference: str (optional, one of USD/EUR/GBP/JPY/AUD)
    ...
    if request.content_type and "multipart/form-data" in request.content_type:
        ...
        currency_preference = put_data.get("currency_preference", "").strip()
        ...
        try:
            ...
            currency_preference = data.get("currency_preference", "").strip()
            profile_image = None
        except (json.JSONDecodeError, AttributeError):
            return json_response(error="Invalid request data", status=400)
    ...
    if currency_preference:
        ...
        if currency_preference in valid_currencies:
